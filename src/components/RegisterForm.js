...
const createUserMutation = loader('../network/graphql/create-user.gql')
const bookingsQuery = loader('../network/graphql/bookings.gql')
...
function RegisterForm() {
  ...
  const { data } = useQuery('bookings', () => {
    return client.request(bookingsQuery, { page: 1, limit: 10 })
  })
  const {
    mutate: createUser,
    isLoading,
    ...
    >
      {({ errors, touched }) => (
        <Form>
          ...
          <div className="form-group">
            <label htmlFor="address">{t('loginPage.address')}</label>
            <Field
              name="address"
              type="text"
